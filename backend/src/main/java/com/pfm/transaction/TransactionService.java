...
public class TransactionService {
    ...
    public void updateTransaction(long transaction_id, Transaction transaction) {
        Optional<Transaction> transactionsFromDb = getTransactionById(transaction_id);

        if (!transactionsFromDb.isPresent()) {
            throw new IllegalStateException("Transaction with id: " + transaction_id + " does not exist in database");
        }
